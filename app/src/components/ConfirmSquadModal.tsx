 ...
 * `defaultBank`/`defaultFreeTransfers` should reflect the real current
 * state once a squad is already confirmed - re-confirming replaces it
 * wholesale, so silently defaulting back to 0/1 would regress it. */
export function ConfirmSquadModal({
  squadSize,
  defaultBank,
  defaultFreeTransfers = 1,
  alreadyConfirmed = false,
  onConfirm,
  onClose,
}: Props) {
  const [bankInput, setBankInput] = useState((defaultBank / 10).toFixed(1))
  const [freeTransfers, setFreeTransfers] = useState(defaultFreeTransfers)
  ...
  return (
    <BottomSheet onClose={onClose}>
      ...
      <p className="text-sm text-white/60 mb-5">
        ...
        {alreadyConfirmed && ' This replaces your currently declared squad, bank and free transfers wholesale - use it to start over, not for routine weekly transfers.'}
      </p>
      ...
      <input
        ...
        className="w-full rounded-xl bg-white/10 px-3 py-2.5 text-base mb-4 outline-none"
      />
