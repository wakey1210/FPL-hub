...
export function PlayerRow({ player, onClick }: { player: PlayerEV; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 py-2.5 px-3 rounded-xl bg-[#1e1e2a] active:bg-[#2a2a38]"
    >
      <span
      ...
      {player.status !== 'a' && <span className="w-2 h-2 rounded-full bg-rose-500 shrink-0" />}
      <div className="text-right shrink-0">
        <p className="text-sm font-bold text-[#00ff87]">{player.total_ev.toFixed(1)}</p>
